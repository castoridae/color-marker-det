...
//
//
//Mat view = Mat::zeros( 1, 1, CV_8UC3 );
//Mat lastView = view;
//CvPoint lastPoint;
//CvPoint point;
//int drag = 0;
...
//
//			cout << "B:G:R avg intensity: " << blue << ":" << green << ":" << red << endl;
//		}
//
//		drag = 0;
//	}
...
//	setMouseCallback("Image View", mouseHandler, NULL);
//
//	help();
//	Settings s;
...
//			float rot = 0.0f;
//			Point3f _3DPoint;
//			float y = (((s.boardSize.height - 1.0f) / 2.0f) * a) + (a / 2.0f);
//			float x = 0.0f;
//			for (int h = 0; h < s.boardSize.height; h++, y += a) {
//				x = (((s.boardSize.width - 2.0f) / 2.0f) * (-a)) - (a / 2.0f);
//				for (int w = 0; w < s.boardSize.width; w++, x += a) {
//					_3DPoint.x = x;
//					_3DPoint.y = y;
//					_3DPoint.z = 0.0f;
//					boardPoints.push_back(_3DPoint);
//				}
//			}
//
//			Mat rvec(3, 1, CV_64F);
//			Mat tvec(3, 1, CV_64F);
////				cout << cameraMatrix.empty() << endl;
//			if (cameraMatrix.empty() != 1) {
//				solvePnP(Mat(boardPoints), Mat(pointBuf), cameraMatrix,
